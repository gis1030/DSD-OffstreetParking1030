...
\- 🎨 \*\*Type de parking\*\* — open-air, covered underground, covered above-ground, boxes (in-block or street-front), mixed configurations, unknown

\- 🤝 \*\*Mutualisation\*\* — public, commercial parking operation, mutualised via BePark, not mutualised (based on `Pk\_public` / `Pk\_com` / `dont\_mutBP`)

\- 🏢 \*\*Type de propriétaire\*\* — public / private / unknown

\- ⭐ \*\*Sites retenus\*\* — site with at least one place in the final 30- or 10-place selection phase (`30\_FINAL` / `10\_FINAL`)
...
\- 🅿️ \*\*21,099 parking places\*\* identified in total (sum of `NB\_TOTAL`)

\- 🏠 \*\*418 private sites\*\* vs. \*\*57 public sites\*\*

\- 🤝 \*\*42 sites\*\* mutualised (3 public, 13 commercial, 26 via BePark) vs. \*\*433\*\* not mutualised

\- ⭐ \*\*32 sites\*\* retained in the final selection (`30\_FINAL` / `10\_FINAL`) vs. \*\*443\*\* not retained

\- ❓ \*\*21 sites\*\* with capacity not yet confirmed ("Potentialité de parking à confirmer")
...
Each site can be clicked to open a detail panel — the selected marker is highlighted (white fill, coloured outline matching its category) to confirm the selection. The panel shows generalities (address, total places, parking type), functions served (residential, offices, retail, industry, education, culture/sport, hotel, senior housing, public parking), additional info (BePark refusal, ownership, occupancy rate, RRU zone, COBRACE authorisation, quartier), data sources, comments, and linked PDF reports where available.
...
│   ├── QuartiersSchaerbeek\_1.js          # Administrative districts

│   ├── LimitesSchaerbeek\_2.js            # Municipal boundary

│   ├── geojson/                          # Raw GeoJSON exports (source data)

│   └── old/                              # Excel source (BD\_20220629\_V2.xlsm, macro-generated parking-data.js) and legacy exports — not deployed
...
├── markers/                               # Address-search result marker icon (1030\_Icon.png)

├── pdf/                                   # Linked site reports (rapport\_PhX\_NN.pdf)
...
| 11/07/2026 | Fixed the Excel-macro data pipeline (accent-encoding bug); corrected the classification criteria for Mutualisation, Sites retenus and Type de propriétaire to match the reference site; redesigned the detail panel (freeform generalities, per-field display rules, corrected labels); added the address-search result marker, marker-selection highlight, and clickable PDF report links |
