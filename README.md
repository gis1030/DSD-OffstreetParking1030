...
- 🤝 **Mutualisation** — public, commercial parking operation, mutualised via BePark, not mutualised (based on `Pk_public` / `Pk_com` / `dont_mutBP`)
- 🏢 **Type de propriétaire** — public / private / unknown
- ⭐ **Sites retenus** — site with at least one place in the final 30- or 10-place selection phase (`30_FINAL` / `10_FINAL`)
...
- 🅿️ **21,099 parking places** identified in total (sum of `NB_TOTAL`)
- 🏠 **418 private sites** vs. **57 public sites**
- 🤝 **42 sites** mutualised (3 public, 13 commercial, 26 via BePark) vs. **433** not mutualised
- ⭐ **32 sites** retained in the final selection (`30_FINAL` / `10_FINAL`) vs. **443** not retained
...
│   └── old/                              # Excel source (BD_20220629_V2.xlsm, macro-generated parking-data.js) and legacy exports — not deployed
...
├── markers/                               # Address-search result marker icon (1030_Icon.png)
├── pdf/                                   # Linked site reports (rapport_PhX_NN.pdf)
